...
"""
    ...
    try:
        result = target.run(encoded_powershell(ps), timeout=config['runtime']['agent_timeout_seconds'])
        timed_out = False
    except TimeoutError:
        raise
    ...
    if result is not None:
        raw_stdout = result.stdout.decode('utf-8', 'replace')
        raw_stderr = result.stderr.decode('utf-8', 'replace')
